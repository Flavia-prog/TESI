...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="AIJack gradient inversion attack on trained BloodMNIST FedAvg results"
    )
...
def load_bloodmnist_train(data_dir: str):
...
def dirichlet_split_indices(
    ...
    concatenated = (
        np.concatenate(split_indices) if split_indices else np.array([], dtype=int)
    )
...
def build_client_indices(train_dataset, config: dict) -> list[np.ndarray]:
...
def build_attack_dataloader(
    ...
    if client_id < 0 or client_id >= len(client_indices):
        raise ValueError(
            f"Invalid client_id {client_id}. Expected [0, {len(client_indices) - 1}]."
        )
    ...
    selected_indices = chosen_client_indices[
        sample_index : sample_index + attack_batch_size
    ]
...
def clean_state_dict_for_plain_model(state_dict: dict) -> dict:
    """
    AIJack's FedAVGServer may save the global model with keys like:
        server_model.conv1.weight

    A plain BloodMNISTCNN expects:
        conv1.weight

    This function strips the `server_model.` prefix when needed.
    """
...
def build_manager(args: argparse.Namespace, device: torch.device):
    """
    Build AIJack's GradientInversionAttackServerManager using the documented
    argument names from the official AIJack tutorial.

    The official AIJack tutorial uses:
    ...
    try:
        ...
        # Fallback for AIJack versions that do not accept `device`.
...
def find_reconstruction_tensor(obj: Any) -> torch.Tensor | None:
    """
    AIJack attack_results can be nested depending on the attack/server wrapper.
    This function searches for a tensor that looks like reconstructed image data.
    """
...
def main() -> None:
    ...
    train_dataset = load_bloodmnist_train(config["data_dir"])
    ...
    server_model = BloodMNISTCNN(num_classes=NUM_CLASSES).to(device)
    server_model.load_state_dict(state_dict)

    manager, manager_kwargs = build_manager(args, device)
    ...
    save_image(
        ...
        output_dir / "original_images.png",
